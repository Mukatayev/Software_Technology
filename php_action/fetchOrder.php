...
$sql = "SELECT order_id, order_date, client_name, client_address,  client_contact, payment_type FROM orders WHERE order_status = 1";
...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	if($row[5] == 1) { 		
 		...
 		$paymentStatus = "<label class='label label-info'>In Transit</label>";
 	} else { 		
 		$paymentStatus = "<label class='label label-warning'>Failed Attempt</label>";
 	} // /else
