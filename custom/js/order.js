...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#editOrderForm").unbind('submit').bind('submit', function() {
			...
			if(clientAddress == "") {
				$("#clientAddress").after('<p class="text-danger"> The Client Address field is required </p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientAddress && clientContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
					// create order button
					// $("#createOrderBtn").button('loading');
